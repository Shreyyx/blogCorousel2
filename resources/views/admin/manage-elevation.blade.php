...
@section('content')
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-header">
                        <h2 class="modal-title" id="sign_up_popupLabel2">Add elevation</h2>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="twm-tabs-style-2">
                            <form id="f" action="{{ 'save-elevation' }}" method="POST"
    ...
    @foreach ($data as $elevations)
        <div style="width:100%;" class="modal fade twm-sign-up" id="editModal{{ $elevations->id }}" tabindex="-1"
            role="dialog" aria-labelledby="exampleModalLabel{{ $elevations->id }}" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel{{ $elevations->id }}">Edit elevation</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <!-- Form for editing blog -->
                        <form id="editForm{{ $elevations->id }}"
                            action="{{ route('update-elevation', ['id' => $elevations->id]) }}" method="POST"
                            enctype="multipart/form-data">
                            @csrf
                            <div class="tab-content" id="myTab2Content">
                                <!--Login Candidate Content-->
                                        ...
                                        <div class="col-lg-12">
                                            <div class="form-group mb-3">
                                                <label for="name"><b>name<span class="text-danger">
                                                            *</span></b></label>
                                                <input name="name" type="text" id="name" class="form-control"
                                                    value="{{ $elevations->name }}" required>
                                            </div>
                                        </div>
                                        <div class="col-lg-12">
                                            <div class="form-group mb-3">
...
@section('jscontent')
    ...
    <script type="text/javascript">
        $(document).on('click', '.delete', function() {
            ...
            var url = "{{ route('delete-elevation') }}";
            swal.fire({
                title: "Are you Sure?",
